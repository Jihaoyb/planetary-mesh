...
| Scheduling | Dispatch picks the first healthy node. | It ignores load, capabilities, queue depth, and historical reliability. | Add node capabilities/load reporting and scheduler policy after queued-job behavior is reliable. | Phase 1 |
| Queued jobs | Jobs can remain queued if no healthy node exists at submission time. | Operators may expect jobs to run once a node later becomes healthy. | Add queued-job scheduler/re-dispatch loop. | Phase 1 |
| Cross-node retry | Retryable dispatch failures retry the selected node, not a different healthy node. | A bad selected node can cause avoidable job failure. | Add cross-node reassignment after dispatch failure. | Phase 1 |
